...
hist(beta1_samples, main = "beta", breaks = 100, xlab = "beta")
```

```{r}
plot(1, type = "n", xlim = c(-9, 9), ylim = c(0,1),
     xlab = "OEFF", ylab = "Prob", main = "Probability with OEFF")
lines(deltaOEFF_grid, prob_grid[1, ], col = 'black')
...
for (i in 1:4000){
  simu_i_prob <- pairwise_data %>% 
    mutate(p = exp(d_OEFF * beta1_samples[i])/ (1 + exp(d_OEFF * beta1_samples[i])))
